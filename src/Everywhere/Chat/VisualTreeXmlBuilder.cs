...
)
{
    ...
    public string BuildXml(CancellationToken cancellationToken)
    {
        ...
        _debugRecorder = new VisualTreeRecorder(coreElements, approximateTokenLimit, "BFS");
#endif
        ...
        while (buildQueue.Count > 0)
        {
            ...
            {
#if DEBUG
                _debugRecorder.RecordStep(buildQueue.Peek().Element, "Stop", 0, "Token limit reached", accumulatedTokenCount, buildQueue.Count);
#endif
                break;
            }
...
#if DEBUG
            _debugRecorder.RegisterNode(element);
#endif

            if (visitedElements.ContainsKey(id))
            {
#if DEBUG
                _debugRecorder.RecordStep(element, "Skip", 0, "Already visited", accumulatedTokenCount, buildQueue.Count);
#endif
                continue;
            }
...
#if DEBUG
            _debugRecorder.RecordStep(element, "Visit", 0, $"Origin: {queueOrigin}", accumulatedTokenCount + tokenCount, buildQueue.Count);
#endif
...
#if DEBUG
                    _debugRecorder.RecordStep(parent, "Enqueue", 0, "Parent", accumulatedTokenCount, buildQueue.Count);
#endif
                }
            }
...
#if DEBUG
                    _debugRecorder.RecordStep(previousSibling, "Enqueue", 0, "PreviousSibling", accumulatedTokenCount, buildQueue.Count);
#endif
                }
            }
...
#if DEBUG
                    _debugRecorder.RecordStep(nextSibling, "Enqueue", 0, "NextSibling", accumulatedTokenCount, buildQueue.Count);
#endif
                }
            }
...
#if DEBUG
                    _debugRecorder.RecordStep(firstChild, "Enqueue", 0, "FirstChild", accumulatedTokenCount, buildQueue.Count);
#endif
                }
            }
        }
